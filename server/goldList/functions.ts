...
import type { Competitor, GoldList, PlayerDetails, PlayerInformation, PlayerItemsOfInterest, VaultInformation } from './types';
import { itemsOfInterest } from './itemsOfInterest';
...
export const getEligiblePlayers = async (ids: number[]): Promise<PlayerDetails[]> => {
    ...
    const contestStartDate = new Date(currentMonth === 11 ? currentYear : currentYear - 1, 11, 1);

    return responses.filter(player => new Date(player.accessDate) >= contestStartDate);
};


export const getPlayerDetails = async (id: number): Promise<PlayerDetails> => {
    const request = await fetch(`https://account.battleon.com/charpage/details?id=${id}`);
    const response = await request.json();
...
export const getCharacterPageItems = (document: Document): PlayerItemsOfInterest => {
    const items = {};

    const textElements = Array.from(document.querySelectorAll('text'));

    Object.keys(itemsOfInterest).forEach(key => {
        const count = textElements.filter(textElement => textElement.innerHTML.trim() === key).length;
        items[key] = count;
    });
...
export const getPlayerInformation = async (id: number, detail: PlayerDetails, competitors: Competitor[]): Promise<PlayerInformation> => {
    const { window: { document } } = await getCharacterPage(id);
    const vaultIdSelector = Array.from(document.querySelectorAll('b'))
        .filter(x => x.innerHTML === 'View Contents');

    // @ts-ignore this will find the expected anchor element
    const vaultId = vaultIdSelector.length ? Number(vaultIdSelector[0].parentElement.href.split('=')[1]) : null;

    const items = await getCharacterPageItems(document);
    const currentDate = new Date();
    const contestIsActive = currentDate.getMonth() === 0 || currentDate.getMonth() === 11;
    const foundCompetitor = competitors.find(competitor => competitor.id === id);
...
export const getPrimaryAccount = (competitors: Competitor[], associatedPlayers: PlayerInformation[]): number | undefined => {
    ...
    const competitivePlayers = competitors.filter(x => associatedPlayerIds.includes(x.id));

    if (competitivePlayers.length) {
        const sorted = associatedPlayers
            .sort((a, b) => {
...
export const calculateTotalGoldFromPlayer = (player: PlayerInformation, otherPlayers: PlayerInformation[], vault?: PlayerItemsOfInterest, isPrimary?: boolean): number => {
    let total = player.gold + player.goldDonated;

    if (isPrimary) {
        return total + otherPlayers
...
export const processVaultInformation = async (vault: VaultInformation, competitors: Competitor[]): Promise<VaultInformation> => {
    const primaryAccountId = getPrimaryAccount(competitors, vault.associatedPlayers);
    const { window: { document } } = await getCharacterPage(vault.vaultId);
